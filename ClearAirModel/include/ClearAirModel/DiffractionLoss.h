...
public:
    ...
    //Get diffraction Loss not exceeded for p=50% (dB)
    inline double getDiffractionLoss_median_dB() const{ return m_diff_loss_median_dB; }
    //Get diffraction Loss not exceeded for p percent of time (annual) (dB)
...
private:
    ...
    /// @brief Diffraction Loss model from Section 4.5.4
    /// @return Returns median diffraction loss (not exceeded for 50 percent of time)
