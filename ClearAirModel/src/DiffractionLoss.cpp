...
DiffractionLoss::DiffractionLoss(const PathProfile::Path& path, const double& height_tx_asl_m, const double& height_rx_asl_m,
    ...
    m_diff_loss_median_dB = calcDiffractionLoss_median_dB();
...
double DiffractionLoss::calcDiffractionLoss_p_percent_dB() const{
    ...
    if(m_p_percent==50){
        ...
        return MathHelpers::interpolate1D(m_diff_loss_median_dB, diffractionLoss_b0percent_dB, Fi);
    }
}
...
double DiffractionLoss::calcSphericalEarthDiffraction_firstTerm_dB(const double& eff_radius_km) const{
    ...
    const double loss_firstTerm_sea_dB = calcSphericalEarthDiffraction_firstTerm_helper_dB(80,5,eff_radius_km);
...
double DiffractionLoss::calcSphericalEarthDiffraction_firstTerm_helper_dB(const double& relPermittivity, 
    ...
    if(m_pol!=Enumerations::PolarizationType::HorizontalPolarized){
        ...
        if(m_pol == Enumerations::PolarizationType::VerticalPolarized){
            ...
            std::ostringstream oStrStream;
            std::cerr << "WARNING: DiffractionLoss::calcSphericalEarthDiffraction_firstTerm_helper_dB(): " 
