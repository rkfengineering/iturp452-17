...
ITUR_P452::DiffractionLoss::DiffractionLoss(const CommonInputs& commonInputs, const double& deltaN, 
    const ItuModels::Enumerations::PolarizationType& pol):
    m_commonInputs{commonInputs}, m_deltaN{deltaN}, m_pol{pol}{
    ...
    m_eff_height_itx_m = m_commonInputs.height_tx_asl_m - eff_terrainHeight_itx_asl_m;
    m_eff_height_irx_m = m_commonInputs.height_rx_asl_m - eff_terrainHeight_irx_asl_m;   
}
...
void ITUR_P452::DiffractionLoss::calcDiffractionLoss_dB(double& out_diff_loss_median_dB, double& out_diff_loss_p_percent_dB) const{
    ...
    if(m_commonInputs.p_percent>50 || m_commonInputs.p_percent < 0.001){
		...
		oStrStream << "ERROR: DiffractionLoss::calcDiffractionLoss_p_percent_dB(): " << 
			...
			<< m_commonInputs.p_percent << " %!" << std::endl;
		throw std::domain_error(oStrStream.str());
	}

    if(m_commonInputs.p_percent==50){
        ...
        double Fi = 1;
        if(m_commonInputs.p_percent>m_commonInputs.timePercentBeta0){
            Fi = CalculationHelpers::inv_cum_norm(m_commonInputs.p_percent/100.0)
                    /CalculationHelpers::inv_cum_norm(m_commonInputs.timePercentBeta0/100.0); //Eq 41a
        }
...
double ITUR_P452::DiffractionLoss::calcDeltaBullingtonLoss_dB(const double& eff_radius_p_km) const{
    ...
    const double Lbulla = calcBullingtonLoss_dB(m_commonInputs.path, m_commonInputs.height_tx_asl_m, 
                m_commonInputs.height_rx_asl_m, eff_radius_p_km);
...
double ITUR_P452::DiffractionLoss::calcBullingtonLoss_dB(const PathProfile::Path& path, const double& height_tx_asl_m,
    ...
    const double wavelength_m = CalculationHelpers::convert_freqGHz_to_wavelength_m(m_commonInputs.freq_GHz);
    ...
    for(auto cit = path.cbegin()+1; cit<path.cend()-1;++cit){
        pt_tx = *cit;
        slope_tx = (pt_tx.h_asl_m+500*Ce*pt_tx.d_km*(m_commonInputs.d_tot_km-pt_tx.d_km)-height_tx_asl_m)/pt_tx.d_km;
        max_slope_tx = std::max(max_slope_tx,slope_tx); 
    }

    //Eq 15 Slope of line from Tx to Rx assuming LOS
    const double slope_tr_los = (height_rx_asl_m-height_tx_asl_m)/m_commonInputs.d_tot_km;
    //Case 1 LOS m_path
    if(max_slope_tx<slope_tr_los){
        ...
        for(auto cit = path.cbegin()+1; cit<path.cend()-1;++cit){
            pt = *cit;
            delta_d = m_commonInputs.d_tot_km-pt.d_km;
            //Note. There is no floor operation in this equation. The square brackets may not be rendered correctly. See Eq 155a
            v1 = (pt.h_asl_m+500.0*Ce*pt.d_km*(delta_d)-(height_tx_asl_m*(delta_d)+height_rx_asl_m*pt.d_km)/m_commonInputs.d_tot_km);
            v2 = std::sqrt(0.002*m_commonInputs.d_tot_km/(wavelength_m*pt.d_km*delta_d));
    ...
    //Case 2 Transhorizon m_path
        ...
        for(auto cit = path.cbegin()+1; cit<path.cend()-1;++cit){
            pt_rx = *cit;
            slope_rx = (pt_rx.h_asl_m+500*Ce*pt_rx.d_km*(m_commonInputs.d_tot_km-pt_rx.d_km)-height_rx_asl_m)
                        /(m_commonInputs.d_tot_km-pt_rx.d_km); 
            max_slope_rx = std::max(max_slope_rx,slope_rx); 
        }

        //Eq 19 distance from bullington point to tx
        const double dbp = (height_rx_asl_m-height_tx_asl_m+max_slope_rx*m_commonInputs.d_tot_km)/(max_slope_tx+max_slope_rx); 

        //Eq 20 diffraction parameter nu at bullington point
        const double nub = (height_tx_asl_m+max_slope_tx*dbp
                    -(height_tx_asl_m*(m_commonInputs.d_tot_km-dbp)+height_rx_asl_m*dbp)/m_commonInputs.d_tot_km) *
                    std::sqrt(0.002*m_commonInputs.d_tot_km/(wavelength_m*dbp*(m_commonInputs.d_tot_km-dbp)));
    ...
    return loss_knifeEdge_dB + (1-std::exp(-loss_knifeEdge_dB/6.0))*(10+0.02*m_commonInputs.d_tot_km); 
}

double ITUR_P452::DiffractionLoss::calcSphericalEarthDiffractionLoss_dB(const double& eff_radius_p_km) const{

    const double wavelength_m =  CalculationHelpers::convert_freqGHz_to_wavelength_m(m_commonInputs.freq_GHz); //wavelength in m
    ...
    if(m_commonInputs.d_tot_km>=d_los_km){
    ...
    const double m = 250*m_commonInputs.d_tot_km*m_commonInputs.d_tot_km/(eff_radius_p_km*(m_eff_height_itx_m+m_eff_height_irx_m)); //Eq 25e
    ...
    const double dse1 = m_commonInputs.d_tot_km/2*(1+b); //Eq 25a
    const double dse2 = m_commonInputs.d_tot_km - dse1; //Eq 25b
    ...
    const double h_se = ((m_eff_height_itx_m - 500.0*dse1*dse1/eff_radius_p_km)*dse2 + 
                        ...
                        /m_commonInputs.d_tot_km; //Eq 24

    //Equation 26 Required Clearance for zero diffraction loss
    const double h_req_m = 17.456 * std::sqrt(dse1*dse2*wavelength_m/m_commonInputs.d_tot_km);
    if(h_se>h_req_m){
        return 0.0;
    }

    //modified effective earth radius
    const double mod_effEarthRadius_km = 500*ItuModels::MathHelpers::simpleSquare(m_commonInputs.d_tot_km/
                (std::sqrt(m_eff_height_itx_m)+std::sqrt(m_eff_height_irx_m))); //Eq 27
...
double ITUR_P452::DiffractionLoss::calcSphericalEarthDiffraction_firstTerm_dB(const double& eff_radius_km) const{
    ...
    //Equation 29
    return ItuModels::MathHelpers::interpolate1D(loss_firstTerm_land_dB, loss_firstTerm_sea_dB, m_commonInputs.fracOverSea);
}
...
double ITUR_P452::DiffractionLoss::calcSphericalEarthDiffraction_firstTerm_singleZone_dB(const double& relPermittivity, 
    ...
    double K = 0.036*std::pow((eff_radius_km*m_commonInputs.freq_GHz),-1.0/3.0)
                *std::pow((ItuModels::MathHelpers::simpleSquare(relPermittivity-1.0)+
                ItuModels::MathHelpers::simpleSquare(18.0*conductivity/m_commonInputs.freq_GHz)),-1.0/4.0); //Eq 30a
    ...
    if(m_pol!=ItuModels::Enumerations::PolarizationType::HorizontalPolarized){
        ...
        const double K_ver = K*std::sqrt(
                    ...
                    + ItuModels::MathHelpers::simpleSquare(18.0*conductivity/m_commonInputs.freq_GHz)
        );
    ...
    //Normalized Distance
    const double X = 21.88 * beta_dft * std::pow((m_commonInputs.freq_GHz/(eff_radius_km*eff_radius_km)),1.0/3) 
                * m_commonInputs.d_tot_km; //Eq 32
    
    //Eq 33,36
    const double beta_times_Y = 0.9575*beta_dft*beta_dft*std::pow(m_commonInputs.freq_GHz*m_commonInputs.freq_GHz/eff_radius_km,1.0/3);
    const double Bt = beta_times_Y*m_eff_height_itx_m;
    const double Br = beta_times_Y*m_eff_height_irx_m;
...
ITUR_P452::TxRxPair ITUR_P452::DiffractionLoss::calcSmoothEarthTxRxHeights_DiffractionModel_amsl_m() const{
    ...
    const double d_tot = m_commonInputs.path.back().d_km; //assume distances start at 0
    ...
    for (auto cit = m_commonInputs.path.begin()+1; cit<m_commonInputs.path.end()-1; ++cit){
        point = *cit;
        delta_d = d_tot-point.d_km;
        height_val = point.h_asl_m-(m_commonInputs.height_tx_asl_m*delta_d+m_commonInputs.height_rx_asl_m*point.d_km)/d_tot; //Eq 165d
    ...
    double eff_height_tx_amsl_m = std::min(m_commonInputs.path.front().h_asl_m, height_smooth_tx_amsl_m); //Eq 167 a,b
    double eff_height_rx_amsl_m = std::min(m_commonInputs.path.back().h_asl_m, height_smooth_rx_amsl_m); //Eq 167 c,d
