...
#include "ITUR_P452/BasicProp.h"
#include "ITUR_P452/DiffractionLoss.h"
#include "ITUR_P452/TropoScatter.h"
...
TEST(HelpersTests, calcMedianEffRadiusTest){
    const double INPUT_DN = 53;
    const double EXPECTED_AE = 9617.759615;
	const double VAL_AE = Helpers::calcMedianEffectiveRadius_km(INPUT_DN);
...
TEST(HelpersTests, calcHorizonAnglesAndDistances_TranshorizonTest){
    ...
    const double EFF_RADIUS_MED_KM = Helpers::calcMedianEffectiveRadius_km(DN);
    ...
    const auto [HorizonAngles, HorizonDistances] = Helpers::calcHorizonAnglesAndDistances(
        p,
        HTS_MASL,
        HRS_MASL,
        EFF_RADIUS_MED_KM,
...
TEST(HelpersTests, calcPathAngularDistance){
    const double INPUT_DIST = 109;
    const double INPUT_THETA_T = -0.6342118;
    const double INPUT_THETA_R = -1.390039674;

    const double INPUT_DN = 53;
	const double MEDIAN_EFFECTIVE_RADIUS = Helpers::calcMedianEffectiveRadius_km(INPUT_DN);

    const double VAL_THETA = Helpers::calcPathAngularDistance_mrad(
        ITUR_P452::TxRxPair{INPUT_THETA_T,INPUT_THETA_R},INPUT_DIST,MEDIAN_EFFECTIVE_RADIUS
    );
