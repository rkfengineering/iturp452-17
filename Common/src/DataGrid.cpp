...
DataGrid::DataGrid(const BinaryFileReader& source, 
			const std::string& fileName, const double& resolution_deg,
	...
	// Edge case: There is no extra column padding for the default ITU data grid
	...
	try {
		_dataGrid = source.readGridData(fileName);
	}
	...
	if (_dataGrid.size() != EXPECTED_NUM_ROWS) {
		...
		oStrStream << "ERROR: DataGrid::DataGrid(): In file \"" 
					<< fileName << "\": expected rows " << EXPECTED_NUM_ROWS << ", but read in " << _dataGrid.size();
		throw std::runtime_error(oStrStream.str());
	}
	if (_dataGrid[0].size() != EXPECTED_NUM_COLUMNS) {
		std::ostringstream oStrStream;
		oStrStream << "ERROR: DataGrid::DataGrid(): In file \"" 
					<< fileName << "\": expected columns " << EXPECTED_NUM_COLUMNS << ", but read in " << _dataGrid[0].size();
		throw std::runtime_error(oStrStream.str());
	}
}


std::vector<DataGridHelpers::BoundingBoxGridPoint> DataGrid::getBoundingBoxList(const GeodeticCoord& location) const {
...
double DataGrid::interpolate2D(const GeodeticCoord& location, const std::vector<double>& customWeightList) const {
	const auto BOUNDING_BOX_INTEGER_PAIRS = DataGridHelpers::calculateBoundingBoxIntegerPairs(location, _resolution_deg,
		_startLat_deg, _endLat_deg, _startLon_deg, _endLon_deg);
...
double DataGrid::interpolate2D(const GeodeticCoord& location) const {
	const auto BOUNDING_BOX_INTEGER_PAIRS = DataGridHelpers::calculateBoundingBoxIntegerPairs(location, _resolution_deg,
		_startLat_deg, _endLat_deg, _startLon_deg, _endLon_deg);
...
double DataGrid::interpCubic(const GeodeticCoord& location) const {
	const auto BOUNDING_BOX_INTEGER_PAIRS = DataGridHelpers::calculateBoundingBoxIntegerPairs(location, _resolution_deg,
		_startLat_deg, _endLat_deg, _startLon_deg, _endLon_deg);
